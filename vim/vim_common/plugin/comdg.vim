...
function! <SID>GetBasename()
    "let fullpath = expand("%:p")
    let pathname = system("basename `pwd`")
    if (pathname[strlen(pathname)-1]=="\n")
        let pathname = strpart(pathname, 0, strlen(pathname)-1)
    return pathname
endfunction
...
function! <SID>InsertHeadFormat()
    ...
    call append(9, "#ifndef  __". s. "_")
    call append(10, "#define  __". s. "_")
    call append(line('$')-2, "#endif  //__". s. "_")
endfunction

function! <SID>InsertGolangHead()
    call <SID>GetDoxFH(1)
    call <SID>InsertEmptyLine(10)

    let s = <SID>GetFileName()
    let file = substitute(s, '.go$', "", "")  		
    if (file=="main")
        let pn = file
    else
        let pn = <SID>GetBasename()
    endif

    let sn = 7
    call append(sn+1, "\/\/ ". pn." is ..." )
    call append(sn+2, "package ". pn)

    call append(sn+4, "import (")
    call append(sn+5,"\t\"\"")
    call append(sn+6,")")
    call cursor(sn+6,4)
...
function! <SID>InsertEmptyLine(linenum)
    let idx = a:linenum
    while (idx > 0)
        call append(line('$') - 1, "")
        let idx = idx - 1
...
" 给函数加注释
map <Leader>ddf :call <SID>GenDoc()<CR>
" 单行注释
map <Leader>ddc :call <SID>ValComment()<CR>
map <Leader>ddcc :call <SID>ValComment1()<CR>
...
"autocmd BufNewFile *.go :call <SID>InsertGolangHead()
